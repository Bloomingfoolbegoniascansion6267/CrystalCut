...
U2NetP는 처리 파이프라인과 배포 구조를 빠르게 검증하기 위한 경량 모델입니다. 머리카락·반투명 물체 같은 최종 제품 품질은 BiRefNet 등 후보 모델을 동일 protocol 뒤에서 비교한 후 결정합니다. EXIF 기반 이름 template, 용량 사전 예측, 취소·재시작 복구와 GPU provider는 다음 단계입니다.
