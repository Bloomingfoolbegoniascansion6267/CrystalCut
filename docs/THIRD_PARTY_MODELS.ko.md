...
Clearcut은 다음 두 양자화 graph를 revision과 SHA-256으로 고정해 최초 사용 시 내려받는다.
...
다운로드한 파일은 앱 데이터 폴더의 `models/slimsam-77-uniform` 아래에 저장한다. Python, Conda, WSL은 필요하지 않으며 Clearcut의 Rust ONNX Runtime worker가 직접 실행한다.
